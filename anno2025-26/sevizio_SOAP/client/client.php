...
if (isset($_POST['qty']) && !empty($_POST['qty']) && isset($_POST['from']) && !empty($_POST['from'])&& isset($_POST['to']) && !empty($_POST['to'])){ 
   ...
   try {
        ...
        $client = new SoapClient($wsdl_url,["location" =>"http://127.0.0.1/servizio_SOAP/server/"]);

        $risposta = $client->converti($qty,$from,$to);

        echo "<div class='jumbotron'><h1>La conversione di ".$qty." ".$from." in ".$to." è ".$risposta."</h1><br></div>";
